...
async function getVkWebSocketSignalServerUrlByJoinIdOrLink(joinIdOrLink) {
...
async function run() {
	const vkWebSocketSignalServer = new VkWebSocketSignalServer(process.env.DEVELOP_VK_WS_URL);
	...
	const servers = await getVkWebSocketSignalServerUrlByJoinIdOrLink(joinIdOrLink);
